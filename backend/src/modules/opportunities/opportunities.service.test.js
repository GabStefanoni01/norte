...
describe('opportunities.service', () => {
  ...
  describe('listar', () => {
      ...
      mockQuery
        .mockResolvedValueOnce({
          rows: [
            {
              id: 1,
              titulo: 'Curso de Java', interesse: 'Tecnologia', estado: null,
              requisitos: ['Java', 'SQL'], idade_minima: null, idade_maxima: null,
            },
            {
              id: 2,
              titulo: 'Curso de Design', interesse: 'Design', estado: null,
              requisitos: ['Figma'], idade_minima: null, idade_maxima: null,
            },
          ],
        })
        .mockResolvedValueOnce({
    ...
    it('considera estado e faixa etária no contexto do match', async () => {
      mockQuery
        .mockResolvedValueOnce({ rows: [{
          id: 10, titulo: 'Programa SP', interesse: 'Tecnologia', estado: 'SP', requisitos: [],
    ...
    it('retorna lacunas de requisitos que não aparecem no perfil', async () => {
      mockQuery
        .mockResolvedValueOnce({ rows: [{
          id: 11, titulo: 'Vaga Java', interesse: 'Tecnologia', estado: null,
  ...
  describe('criar', () => {
    it('rejeita faixa etária inválida', async () => {
      await expect(opportunitiesService.criar({
        titulo: 'Programa', link: 'https://example.com', idade_minima: 25, idade_maxima: 18,
    ...
    it('cria uma oportunidade válida com os valores recebidos', async () => {
      ...
      const resultado = await opportunitiesService.criar({
        titulo: 'Curso de Node', empresa: 'Norte', categoria: 'Tecnologia', tipo: 'curso',
        descricao: 'Curso introdutório', interesse: 'Tecnologia', estado: 'SP', gratuito: true,
        idade_minima: 16, idade_maxima: 30, link: 'https://example.com/curso', requisitos: ['JavaScript'],
      });
  ...
  describe('fecharLacuna', () =>
    it('adiciona os requisitos faltantes ao plano existente', async () => {
      mockQuery
        .mockResolvedValueOnce({ rows: [{
          id: 20, titulo: 'Vaga Backend', interesse: 'Tecnologia', estado: null,
        ...
        .mockResolvedValueOnce({ rows: [{
          id: 8, etapas: [{ mes: 1, titulo: 'Base', itens: [{
            id: 'base1', descricao: 'Estudar lógica', tipo: 'aprender', status: 'concluido',
          }] }],
        }] })
        .mockResolvedValueOnce({ rows: [] });
      ...
      expect(resultado).toEqual({
        message: '1 item(ns) adicionados ao seu plano de evolução.', itensAdicionados: 1,
      });
    ...
    it('não altera o plano quando o usuário já tem 100% de match', async () => {
      mockQuery
        .mockResolvedValueOnce({ rows: [{
          id: 21, titulo: 'Curso Java', interesse: 'Tecnologia', estado: null,
      ...
      expect(resultado).toEqual({
        message: 'Você já tem 100% de match com essa oportunidade!', itensAdicionados: 0,
      });
      expect(mockQuery).toHaveBeenCalledTimes(2);
    });
  });
